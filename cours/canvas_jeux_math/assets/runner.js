...
(function () {
  ...
  function closeFs() {
    ...
    // un jeu "lancé" (caché dans le cours) est arrêté quand on ferme
    if (leaving.classList.contains('launchable')) {
      const d = registry.find(function (r) { return r.el === leaving; });
      if (d) d.stop();
    }
  }

  // Lance un jeu caché : ouvre en plein écran et (re)démarre à zéro
  function launchDemo(demo) {
    openFs(demo);
    const d = registry.find(function (r) { return r.el === demo; });
    if (d) { d.stop(); d.start(); }
  }
  ...
  function setupDemo(demo) {
    ...
    registry.push({ el: demo, start: start, stop: stop, run: run });
  }
  ...
  function init() {
    ...
    // Boutons "Démarrer le jeu" : lancent le jeu ciblé en plein écran
    document.querySelectorAll('.launch').forEach(function (btn) {
      btn.addEventListener('click', function () {
        const demo = document.getElementById(btn.getAttribute('data-target'));
        if (demo) launchDemo(demo);
      });
    });
